...
void CZBuffer::Phong(CDC *pDC,CP3 ViewPoint,CLighting *pLight,CMaterial *pMaterial,int Sign)//填充多边形
{
	...
	for(pCurrentB=pHeadB;pCurrentB!=NULL;pCurrentB=pCurrentB->pNext)
	{
		...
		double Va,Vb;
		Va=Interpolation(pCurrentB->ScanLine,pHeadE->ps.y,pHeadE->pe.y,pHeadE->A,pHeadE->B);
		Vb=Interpolation(pCurrentB->ScanLine,pHeadE->pNext->ps.y,pHeadE->pNext->pe.y,pHeadE->pNext->A,pHeadE->pNext->B);
		...
		for(pT1=pHeadE;pT1!=NULL;pT1=pT1->pNext)
		{
			...
			else
			{
				...
				for(double x=xleft;x<xright;x++)//左闭右开
				{
					...
					VAB=Interpolation(x,xleft,xright,Va,Vb);
					nf=Interpolation(x,xleft,xright,na,nb);
					CRGB c=pLight->Lighting(ViewPoint,CP3(Round(x),pCurrentB->ScanLine,CurDeep),nf,pMaterial,true,CRGB(0.0,0.0,0.0));
					...
					if(CurDeep<=zBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2])//如果当前采样点的深度小于帧缓冲器中原采样点的深度
					{
						if(SPHERE==Sign)//球不透明，写入帧缓冲
						{
							...
							zBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2]=CurDeep;
						...
						{	
							...
							CRGB c1=pLight->Lighting(ViewPoint,CP3(Round(x),pCurrentB->ScanLine,CurDeep),nf,pMaterial,false,ctrsns);

							zBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2]=CurDeep;
							
							
					//		pDC->SetPixelV(Round(x),pCurrentB->ScanLine,RGB(ctrsns.red*255/*+c1.red*255*/,ctrsns.green*255/*+c1.green*255*/,ctrsns.blue*255/*+c1.blue*255*/));
						//	pDC->SetPixelV(Round(x)+500,pCurrentB->ScanLine,RGB(c.red*255+c1.red*255,c.green*255+c1.green*255,c.blue*255+c1.blue*255));	
							pDC->SetPixelV(Round(x),pCurrentB->ScanLine,RGB(c1.red*255,c1.green*255,c1.blue*255));	
						}
					}
					CurDeep+=DeepStep;
				}
...
double CZBuffer::Interpolation(double t,double t1,double t2,double A,double B)
{
	
	VAB=(t-t2)/(t1-t2)*A+(t-t1)/(t2-t1)*B;
	return VAB;
}
CRGB CZBuffer::GetTransColor(CRGB clr1,CRGB clr2)//计算透明颜色
{
	...
	color=(1-VAB)*clr1+VAB*clr2;
	//color.red=(1-VAB)*clr1.red*255+VAB*GetRValue(clr2);
	///*color.green=(1-VAB)*clr1.green*255+VAB*GetRValue(clr2);
	//color.blue=(1-VAB)*clr1.blue*255+VAB*GetRValue(clr2);*/
	return color;
}
void CZBuffer::InitDeepBuffer(CDC *pDC,int Width,int Height,double Depth,CRGB BkClr)//初始化深度与颜色缓存
{
	...
	CPoint BottomLeft(-Width / 2, -Height / 2);
	double p=100,q=100;
	for(int i=0;i<Width;i++)//初始化深度缓存
	{
		for(int j=0;j<Height;j++)
		{
				...
				double f=A*(cos(p*i)+cos(q*j));
				pDC->SetPixelV(i + BottomLeft.x, j + BottomLeft.y, RGB(f * 255, f * 255, f * 255));*/
		//		BkaClr=pDC->GetPixel(i + BottomLeft.x, j + BottomLeft.y);
			
			zBuffer[i][j]=Depth;
		//	cBuffer[i][j].red=GetRValue(BkaClr);
		//	cBuffer[i][j].green=GetGValue(BkaClr);
		//	cBuffer[i][j].blue=GetBValue(BkaClr);
			cBuffer[i][j]=BkClr;
		}
	}

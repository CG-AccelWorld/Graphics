// BeierObject.cpp: implementation of the CBeierObject class.
//
//////////////////////////////////////////////////////////////////////
...
CBezierObject::CBezierObject()
{
	...
	pMaterial1->SetAmbient(CRGB(0.175,0.012,0.012));//材质对环境光光的反射率
	pMaterial1->SetDiffuse(CRGB/*(0.545,0.270,0.074)*/(0.675,0.341,0.041));//材质对漫反射光的反射率
	pMaterial1->SetSpecular(CRGB(1.0,1.0,1.0));//材质对镜面反射光的反射率
	pMaterial1->SetEmit(CRGB(0.2,0.0,0.0));//材质自身发散的颜色
	pMaterial1->SetExp(30);//高光指数
	pMaterial1->M_m=0.325;//材质粗糙度
	pMaterial1->M_f=0.175;//材质菲涅尔系数，表示反射方向上的光强占原始光强的比率
}
...
void CBezierObject::ReadPatch()//读入双三次曲面片
{
	for (int i = 0; i < NumObject; i++)
	{
		for (int m = 0 ; m < 4 ; m++)
		{
			Patch[i][m].SetNum(16);
			for (int j = 0; j < 16; j++)
				Patch[i][m].ptI[j] = j + m * 16;
		}

	}
}

void CBezierObject::DrawObject(CDC* pDC)
{
	for (int i = 0; i < NumObject; i++)
	{
		for (int m = 0; m < 4 ;m++)
		{
			for (int j = 0; j < 16; j++)
			{
				P3[j] = Vertex[i][Patch[i][m].ptI[j]];
			}
	    	BicubicCurvedSurface(pDC,i,m);//双三次Bezier曲面
		}
	}
	zbuf = new CZBuffer();
...
void CBezierObject::BicubicCurvedSurface(CDC* pDC,int NumObject,int m)//绘制双三次Bezier曲面片	
{
	CMatrix44 Mbe;//矩阵Mbe
...
void CBezierObject::DrawFace(CDC *pDC, int NumObject,bool sign)
{
	...
	{ 
		for (int v=0;v<lie;v++) //v 列  竖
		{
			...
			CVector3 vN0,vN1,vN2,vN3,AveN;

			if (0==u)   //第一行
			{

				//左下
				CVector3 v05(FacePoint[u][v],FacePoint[u+1][beforev]);
				CVector3 v06(FacePoint[u][v],FacePoint[u+1][v]);
				vN2 = Cross(v05,v06);
				//右下
				CVector3 v07(FacePoint[u][v],FacePoint[u+1][v]);
				CVector3 v08(FacePoint[u][v],FacePoint[u+1][afterv]);
				vN3 = Cross(v07,v08);
				AveN = (vN2+vN3);
				AveN /= AveN.Mag();
			...
			else  if (hang == afteru)
			{
				...
				//左上
				CVector3 v01(FacePoint[u][v],FacePoint[u-1][v]);
				CVector3 v02(FacePoint[u][v],FacePoint[u-1][beforev]);
				vN0 = Cross(v01,v02);
				//右上
				CVector3 v03(FacePoint[u][v],FacePoint[u-1][afterv]);
				CVector3 v04(FacePoint[u][v],FacePoint[u-1][v]);
				vN1 = Cross(v03,v04);
				AveN = (vN0+vN1);
				AveN /= AveN.Mag();
				FacePoint[u][v].xk = AveN.x;
			...
			else
			{
				...
				//左上
				CVector3 v01(FacePoint[u][v],FacePoint[u-1][v]);
				CVector3 v02(FacePoint[u][v],FacePoint[u][beforev]);
				vN0 = Cross(v01,v02);
				//右上
				CVector3 v03(FacePoint[u][v],FacePoint[u][afterv]);
				CVector3 v04(FacePoint[u][v],FacePoint[u-1][v]);
				vN1 = Cross(v03,v04);
				//左下
				CVector3 v05(FacePoint[u][v],FacePoint[u][beforev]);
				CVector3 v06(FacePoint[u][v],FacePoint[u+1][v]);
				vN2 = Cross(v05,v06);
				//右下
				CVector3 v07(FacePoint[u][v],FacePoint[u+1][v]);
				CVector3 v08(FacePoint[u][v],FacePoint[u][afterv]);
				vN3 = Cross(v07,v08);
	...
	{ 
		for (int v=0;v<lie;v++) //v 列  竖
		{
			...
			DrawFaceP4(pDC,FP4,sign);
		}
	}
}
void CBezierObject::DrawFaceP4(CDC *pDC,CP3 P4[4],bool sign)
{
	CP3 ViewPoint = projection.InitalParameter();
	CVector3 Normal[4];
	//1. 求面的视矢量
	CVector3 ViewVector(P4[0],ViewPoint);
	ViewVector = ViewVector.Normalize();
	//2.求面的法矢量
	CVector3 v01(P4[0],P4[3]);
	CVector3 v02(P4[0],P4[2]);
	...
	CPi3 point4[4];
		for (int i=0;i<4;i++)
		{
			...
			Normal[i] = CVector3(-P4[i].xk,-P4[i].yk,-P4[i].zk);
		}
    	zbuf->SetPoint(point4[0],point4[1],point4[2],Normal[0],Normal[1],Normal[2]);
		zbuf->PhongShading(pDC,ViewPoint,pLight,pMaterial1,A);
		zbuf->SetPoint(point4[0],point4[2], point4[3],Normal[0],Normal[2],Normal[3]);
		zbuf->PhongShading(pDC,ViewPoint,pLight,pMaterial1,A);

		zbuf->SetPoint(point4[0],point4[1],point4[2],Normal[0],Normal[1],Normal[2]);
		zbuf->PhongShading(pDC,ViewPoint,pLight,pMaterial1,B);
		zbuf->SetPoint(point4[0],point4[2], point4[3],Normal[0],Normal[2],Normal[3]);
		zbuf->PhongShading(pDC,ViewPoint,pLight,pMaterial1,B);
}
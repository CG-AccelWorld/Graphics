	...
	for(int nFacet = 0; nFacet<6; nFacet++)
	{
		...
		if(Dot(ViewVector,F[nFacet].fNormal)>=0)//背面剔除
		{
			for(int nPoint=0;nPoint<4;nPoint++)//顶点循环
			{
				...
				Point[nPoint].x=ScreenP.x;
				Point[nPoint].y=Round(ScreenP.y);
				Point[nPoint].c=ScreenP.c;			
			}
			CTriangle triangle(Point[0],Point[1],Point[2]);
			triangle.Gouraud(pDC);
			CTriangle triangle2(Point[0],Point[2],Point[3]);
			triangle2.Gouraud(pDC);

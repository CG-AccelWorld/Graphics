...
void CTriangle::Gouraud(CDC* pDC)
{
	...
	if(point0 != point1 && point0 != point2 && point1 != point2)	//重载了 ！= 运算符
	{
		...
		for(int y = point0.y ; y < point1.y; ++y)//下闭上开
		{
			...
			{
				clr = ClrInterpolation(x, pLeft[n].x, pRight[n].x, pLeft[n].c, pRight[n].c);
...
CRGB CTriangle::ClrInterpolation(double t, double mStart, double mEnd, CRGB cStart, CRGB cEnd)//颜色线性插值
{
	CRGB color;
